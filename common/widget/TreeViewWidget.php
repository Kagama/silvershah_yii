...
//    private $_tempArr = null;
    ...
    public function run() {
        if (empty($this->data))
            throw new InvalidCallException("Свойство \"" . get_called_class() . '::$data" не должно быть пустым');
    ...
    private function prepareSourceArray ($_data, $_parent_id = null) {
        ...
        foreach ($_data as $index => $ar) {
            ...
            if ($ar->$parent_filed_name == $_parent_id) {
                $tempObj = $ar;
                unset($_data[$index]);
                $returnArray[] = ['ar_obj' => $tempObj, 'children' => $this->prepareSourceArray($_data, $tempObj->id)];
            }
        }
        return $returnArray;
    }
